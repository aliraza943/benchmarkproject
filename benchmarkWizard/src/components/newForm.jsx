...
// Component to render after successful submission
import SubmittedPropertyUse from '../components/PropertyUse';
export default function PropertyUse({ id }) {
  ...
  // New state to track submission and returned ID
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [newId, setNewId] = useState(null);

  useEffect(() => {
    fetch('/propertyUse/propertyUseList.xsd')
      .then(res => res.text())
      .then(data => {
        const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '' });
        const json = parser.parse(data);
        const schema = json['xs:schema'] || json['schema'];
        if (!schema) return;
        let includes = schema['xs:include'] || schema['include'];
        includes = Array.isArray(includes) ? includes : [includes];
        const formattedNames = includes.map(inc =>
          inc.schemaLocation
            .replace('.xsd', '')
            .replace(/([A-Z])/g, ' $1')
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.status === 201) {
        // extract ID from responseText
        const match = result.responseText.match(/<id>(\d+)<\/id>/);
        if (match) {
          setNewId(match[1]);
          setIsSubmitted(true);
        }
    ...
    <form onSubmit={handleSubmit}>
      ...
      {refDetails.length > 0 && (
        <button
          ...
          className={`mt-6 px-4 py-2 text-white rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-blue-500 
      ...
      {newId && <SubmittedPropertyUse id={id} />}
